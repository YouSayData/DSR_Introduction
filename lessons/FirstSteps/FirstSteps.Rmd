...
## Welcome to the RAL Data Science with R Course

My name is Thomas Koentges and you can learn more about me on my [company's website](https://www.yousaydata.com). The "First Steps" tutorial is a lightly altered version of the `learnr` package's [example code](https://github.com/rstudio/learnr/tree/master/inst/tutorials/ex-setup-r/ex-setup-r.Rmd).
...
```{r quiz1, echo = FALSE}
question("Check all that you still need to do:",
  answer("install R on my computer", message = "* Install R"),
  answer("install the RStudio IDE", message = "* Install RStudio IDE"),
  answer("install the tidyverse R package", message = "* Install Packages"),
  answer("None of the above. I've done them all.", correct = TRUE, message = "You can skip this tutorial and you are ready to go for the class!"),
  type = "multiple",
...
Please note that the video below is a litte bit older. You should look out for the latest R version for your system (at the point of writing that would be > 4.0.0).
